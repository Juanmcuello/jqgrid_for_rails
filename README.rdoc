...
To create a call to a jqgrid api for the pager that in javascript would be:
...
To generate the grid, you can first create a method in a helper and then call the +to_jqgrid_api+ method to generate the java script code. For example, if you have an invoices_helper.rb file, you can define a method there:
  ...
  module InvoicesHelper
    ...
    def invoices_jqgrid
      ...
      to_jqgrid_api 'invoices_list', grid_options
    end
  end
...
This method creates an active record +order+ string using the params received from the grid.
